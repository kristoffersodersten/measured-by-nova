  ...
  return buildMeasuredUiSurface({
    ...
    panels: [
      ...
      { id: "model-review", title: "Model Review", order: 2, states: [
        { id: "infrastructure", label: `${runtime.environmentTruth.engine} ready via ${runtime.environmentTruth.provider}`, topology: "infrastructure", status: "ready", provenance: runtime.environmentTruth.endpoint, operatorApprovalRequired: false },
...
export function renderWorkspaceHtml(surface: MeasuredUiSurface): string {
  ...
  </style></head><body><main><header class="masthead"><div><p class="eyebrow">Instrument workspace</p><h1>Measured by Nova</h1></div><p>${escapeHtml(validated.outputTruth.technicalLabel)}</p></header>
  <dl class="truth" aria-label="Environment truth"><div><dt>Provider</dt><dd>${escapeHtml(truth.provider)}</dd></div><div><dt>Execution</dt><dd>${escapeHtml(truth.executionGeography)} · ${escapeHtml(truth.latencyClass)}</dd></div><div><dt>Cost</dt><dd>${escapeHtml(truth.costClass)}</dd></div><div><dt>Privacy</dt><dd>${escapeHtml(truth.privacyBoundary)}</dd></div></dl>
  <div class="panels">${panels}</div><aside class="truth-warning"><strong>${escapeHtml(validated.outputTruth.previewLabel)}</strong><p>Preview has no permit or geometry authority. Human review remains required before model lock.</p><button class="override" type="button" id="manual-override">Manual override: hold delivery</button><output id="decision" aria-live="polite"></output></aside>
