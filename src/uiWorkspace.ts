...
const CustomerEvidenceSchema = z.object({
  ...
  measurements: z.array(z.object({ id: z.string().min(1).max(120), label: z.string().min(1).max(160), value: z.number().finite().positive(), unit: z.literal("mm"), confidence: z.enum(["low", "medium", "high"]), source: z.enum(["permit_pdf", "manual_measurement", "photo_inferred", "unknown"]) }).strict()).max(1000),
  materials: z.array(z.object({ id: z.string().min(1).max(120), label: z.string().min(1).max(120), target: z.string().min(1).max(120), confidence: z.enum(["low", "medium", "high"]), source: z.enum(["manual_measurement", "photo_reference", "user_declared"]), verified: z.boolean() }).strict()).max(1000),
...
export function renderWorkspaceHtml(surface: MeasuredUiSurface, heldBy: string | null = null, deliveryArtifacts: Array<{ format: string; sizeBytes: number; url: string }> = [], customerViews: { previewUrl?: string; viewerUrl?: string } = {}, customerEvidence: z.input<typeof CustomerEvidenceSchema> | null = null): string {
  ...
  const customerEvidenceHtml = !heldBy && evidence ? `<section id="customer-evidence" class="customer-evidence" aria-labelledby="customer-evidence-title"><header><div><p class="eyebrow">Public evidence</p><h2 id="customer-evidence-title">${escapeHtml(evidence.trustCategory.split("_").map((part) => part[0]?.toUpperCase() + part.slice(1)).join(" "))}</h2></div><p>No combined seller score</p></header><div class="evidence-grid"><section><h3>Measurements</h3><ul>${evidence.measurements.map((entry) => `<li><strong>${escapeHtml(entry.label)}</strong><span>${entry.value} ${entry.unit} · ${escapeHtml(entry.confidence)} · ${escapeHtml(entry.source)}</span></li>`).join("") || "<li>No declared measurements</li>"}</ul></section><section><h3>Materials</h3><ul>${evidence.materials.map((entry) => `<li><strong>${escapeHtml(entry.label)}</strong><span>${escapeHtml(entry.target)} · ${escapeHtml(entry.confidence)} · ${escapeHtml(entry.source)} · ${entry.verified ? "verified declaration" : "reference declaration"}</span></li>`).join("") || "<li>No declared materials</li>"}</ul></section><section><h3>Known deviations</h3><ul>${evidence.conditions.map((entry) => `<li><strong>${escapeHtml(entry.id)}</strong><span>${escapeHtml(entry.status)}</span></li>`).join("") || "<li>No declared known deviations</li>"}</ul><p>${escapeHtml(evidence.limitation)}</p></section></div></section>` : "";
