...
const CustomerEvidenceSchema = z.object({
  ...
  measurements: z.array(z.object({ id: z.string().min(1).max(120), label: z.string().min(1).max(160), value: z.number().finite().positive(), unit: z.enum(["mm", "deg", "percent"]), tolerance: z.number().finite().positive().optional(), confidence: z.enum(["low", "medium", "high"]), source: z.enum(["permit_pdf", "manual_measurement", "photo_inferred", "unknown", "drawing", "calibrated_anchor"]), claimStatus: z.literal("reference") }).strict()).max(1000),
  ...
  limitation: z.literal("Only declared evidence is shown. Absence of a condition record does not prove absence of defects.")
}).strict();

export function renderWorkspaceHtml(surface: MeasuredUiSurface, heldBy: string | null = null, deliveryArtifacts: Array<{ format: string; sizeBytes: number; url: string }> = [], customerViews: { previewUrl?: string; viewerUrl?: string } = {}, customerEvidence: z.input<typeof CustomerEvidenceSchema> | null = null): string {
  ...
  const validatedViews = z.object({
    previewUrl: z.string().regex(/^\/api\/preview\?projectId=[A-Za-z0-9._%-]+$/).optional(),
    viewerUrl: z.string().regex(/^\/viewer\/[A-Za-z0-9._%-]+\/index\.html$/).optional()
  }).strict().parse(customerViews);
  const evidence = customerEvidence === null ? null : CustomerEvidenceSchema.parse(customerEvidence);
  ...
  const customerViewing = !heldBy && (validatedViews.previewUrl || validatedViews.viewerUrl) ? `<section id="customer-viewing" class="customer-viewing" aria-labelledby="customer-viewing-title"><h2 id="customer-viewing-title">Customer viewing</h2><p>${escapeHtml(validated.outputTruth.previewLabel)}. Measurements and technical artifacts remain separately verified.</p>${validatedViews.previewUrl ? `<img src="${escapeHtml(validatedViews.previewUrl)}" alt="Source-backed photorealistic preview; not verified geometry truth">` : ""}${validatedViews.viewerUrl ? `<a href="${escapeHtml(validatedViews.viewerUrl)}">Open interactive locked-model viewer</a>` : ""}</section>` : "";
  const customerEvidenceHtml = !heldBy && evidence ? `<section id="customer-evidence" class="customer-evidence" aria-labelledby="customer-evidence-title"><header><div><p class="eyebrow">Public evidence</p><h2 id="customer-evidence-title">${escapeHtml(evidence.trustCategory.split("_").map((part) => part[0]?.toUpperCase() + part.slice(1)).join(" "))}</h2></div><p>No combined seller score</p></header><div class="evidence-grid"><section><h3>Measurements</h3><ul>${evidence.measurements.map((entry) => `<li><strong>${escapeHtml(entry.label)}</strong><span>${entry.value} ${entry.unit}${entry.tolerance ? ` ± ${entry.tolerance} ${entry.unit}` : " · tolerance not declared"} · ${escapeHtml(entry.confidence)} · ${escapeHtml(entry.source)} · ${entry.claimStatus}</span></li>`).join("") || "<li>No declared measurements</li>"}</ul></section><section><h3>Materials</h3><ul>${evidence.materials.map((entry) => `<li><strong>${escapeHtml(entry.label)}</strong><span>${escapeHtml(entry.target)} · ${escapeHtml(entry.category ?? "category not declared")} · ${escapeHtml(entry.provenance ?? entry.source ?? "source not declared")} · ${entry.claimStatus}</span>${entry.sourcePhotos?.length ? `<small>Evidence: ${escapeHtml(entry.sourcePhotos.join(", "))}</small>` : ""}</li>`).join("") || "<li>No declared materials</li>"}</ul></section><section><h3>Known deviations</h3><ul>${evidence.conditions.map((entry) => `<li><strong>${escapeHtml(entry.id)}</strong><span>${escapeHtml(entry.type ?? "deviation")} · ${escapeHtml(entry.severity ?? "severity not declared")} · ${escapeHtml(entry.verification ?? entry.status)} · ${escapeHtml(entry.status)}</span>${entry.sourcePhotos?.length ? `<small>Evidence: ${escapeHtml(entry.sourcePhotos.join(", "))}</small>` : ""}</li>`).join("") || "<li>No declared known deviations</li>"}</ul><p>${escapeHtml(evidence.limitation)}</p></section></div></section>` : "";
  ...
  .delivery{display:flex;flex-wrap:wrap;gap:8px;margin-top:16px}.delivery a,.customer-viewing a{color:inherit;border:1px solid var(--line);border-radius:6px;padding:8px 12px;text-decoration:none}.delivery a:focus-visible,.customer-viewing a:focus-visible{outline:3px solid #4d78cc;outline-offset:2px}.customer-viewing,.customer-evidence{display:grid;gap:12px;margin-top:16px;border:1px solid var(--line);border-radius:8px;padding:16px}.customer-viewing img{display:block;max-width:100%;max-height:480px;object-fit:contain;background:#111}.customer-evidence>header{display:flex;justify-content:space-between;gap:16px}.evidence-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.evidence-grid h3{font-size:15px;margin:0}.evidence-grid li{display:grid;gap:4px}.evidence-grid span,.evidence-grid p{color:var(--muted)}@media(max-width:880px){.evidence-grid{grid-template-columns:1fr}}</style></head><body><main><header class="masthead"><div><p class="eyebrow">Instrument workspace</p><h1>Measured by Nova</h1></div><p id="technical-output-state">${escapeHtml(technicalOutput)}</p></header>
