...
export async function loadUiProjectWorkspace(config: UiRuntimeConfig, projectId: string) {
  ...
  surface.panels[1].states = [
    { id: "blender-execution", label: portableExport.status === "ready" ? `Blender export executed · ${portableExport.evidence.requestedFormats.join(", ")}` : "Blender export not verified", topology: "infrastructure", status: portableExport.status === "ready" ? "ready" : portableExport.status === "missing" ? "pending" : "blocked", provenance: project.artifacts.portableExportManifest ?? "portable-export-manifest", ...(portableExport.status === "blocked" ? { blockingReason: portableExport.code } : {}), operatorApprovalRequired: false },
  ...
  surface.panels[2].states = [{ id: "delivery", label: surface.outputTruth.previewLabel, topology: "system", status: deliveryBlockingReason ? "blocked" : portableExport.status === "ready" ? "ready" : "pending", ...(previewEvidence ? { confidence: "medium" as const } : {}), provenance: portableExport.status === "ready" ? project.artifacts.portableExportManifest : previewEvidence ? "validated-render-manifest" : "portable-export-manifest", ...(deliveryBlockingReason ? { blockingReason: deliveryBlockingReason } : {}), operatorApprovalRequired: false }];
