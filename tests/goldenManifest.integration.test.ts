...
describe("golden manifest integration", () => {
  ...
  it("renders a digital viewing preview from a locked Blender source and render manifest", async () => {
    ...
    const previewProjectBase = MeasurementProjectSchema.parse({ schemaVersion: 1, projectId: capture.projectId, unit: "mm", photos: [{ path: capture.photos[0].path, role: "reference", confidence: "high" }], dimensions: [{ label: "overall length", valueMm: 4820, confidence: "high", source: "manual_measurement" }], materialNotes: [{ elementId: "body", material: "automotive paint", confidence: "medium", source: "photo_reference", verified: false }], artifacts: { blend: sourceBlendPath, digitalViewingPreview: manifest.artifacts.render, digitalViewingRenderManifest: manifest.artifacts.manifest } });
    ...
    await writeFile(path.join(projectDir, "project.json"), JSON.stringify({ ...previewProject, modelLock: previewLock, artifacts: { ...previewProject.artifacts, digitalViewingPreviewModelHash: previewLock.modelHash, digitalViewingPreviewSourceProjectHash: previewLock.sourceProjectHash } })); await storeUiManualTrust(outputDir, capture.projectId);
    ...
    try {
      const workspace = await (await fetch(`${origin}/api/workspace?projectId=${capture.projectId}`)).json() as { customerViews: { previewUrl?: string }; customerEvidence: { trustCategory: string; measurements: Array<{ value: number }>; materials: Array<{ label: string }>; conditions: Array<{ id: string; status: string }> } }; expect(workspace.customerViews.previewUrl).toBe(`/api/preview?projectId=${capture.projectId}`);
      expect(workspace.customerEvidence).toMatchObject({ trustCategory: "reference", measurements: [{ value: 4820 }], materials: [{ label: "automotive paint" }], conditions: [{ id: "scratch-1", status: "reference" }] });
      ...
      const customerPage = await (await fetch(`${origin}/?projectId=${capture.projectId}`)).text(); expect(customerPage).toContain("not verified truth"); expect(customerPage).toContain('id="customer-evidence"'); expect(customerPage).toContain("No combined seller score");
      ...
      await fetch(`${origin}/api/operator-decision`, { method: "POST", headers: { origin, "content-type": "application/json" }, body: JSON.stringify({ decision: "hold", actor: "operator", projectId: capture.projectId }) }); expect((await fetch(`${origin}${workspace.customerViews.previewUrl}`)).status).toBe(409); expect(await (await fetch(`${origin}/?projectId=${capture.projectId}`)).text()).not.toContain('id="customer-evidence"');
