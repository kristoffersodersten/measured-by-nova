...
describe("executable Measured workspace", () => {
  ...
  it("serves loopback HTML and explicit workspace state with security headers", async () => {
    ...
    const page = await fetch(origin); const state = await fetch(`${origin}/api/workspace`); const script = await (await fetch(`${origin}/workspace.js`)).text();
    ...
    expect(script).toContain('getElementById("customer-evidence")?.remove()');
  });
  ...
  it("discovers only valid in-root projects and derives causal live state", async () => {
    ...
    expect(workspace.surface.panels[1]?.states[0]).toMatchObject({ id: "portable-delivery", topology: "infrastructure", status: "blocked", blockingReason: "capture_trust_incomplete" });
    expect(workspace.surface.panels[2]?.states[0]).toMatchObject({ id: "preview", label: "Photorealistic preview - not verified truth", status: "pending" });
